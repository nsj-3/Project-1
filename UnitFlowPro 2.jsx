...
const SUPABASE_URL  = "https://jutylnusagvawmqumbne.supabase.co";
const SUPABASE_ANON = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp1dHlsbnVzYWd2YXdtcXVtYm5lIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc0MTU1MzMsImV4cCI6MjA5Mjk5MTUzM30.VZaqIWwV2tQh_tvkLpnE8tU-4ONZakDhrhcOV0ocyhY";
...
const supabase = {
  ...
  async select(table, filter = "") {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${filter}&order=created_at.desc`, { headers: this._h() });
...
function Dashboard() {
  ...
  const ready   = turnovers.filter(t => t.is_ready).length;
  const active  = turnovers.filter(t => !t.is_ready && t.stages.some(s => s.status === "in_progress")).length;
  const overdue = turnovers.filter(t => {
    const d = Math.ceil((new Date(t.target_ready_date) - Date.now()) / 86400000);
    return d < 0 && !t.is_ready;
  }).length;
  const queued  = turnovers.filter(t => !t.is_ready && t.stages.every(s => s.status === "idle")).length;

  // Group by stage for the mini pipeline view
  const stageActivity = MR_STAGES.map(s => ({
    ...s,
    count: turnovers.filter(t => !t.is_ready && t.stages.find(st => st.id === s.id)?.status === "in_progress").length,
  })).filter(s => s.count > 0);

  // Upcoming targets
  const urgent = [...turnovers]
    .filter(t => !t.is_ready)
    ...
    <div style={{ padding: "16px 16px 100px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 20 }}>
        {[
          ...
          { label: "Overdue",  value: overdue, color: overdue > 0 ? "#dc2626" : "#a09890", bg: overdue > 0 ? "#fef2f2" : "#f9f7f4", border: overdue > 0 ? "#fecaca" : "#e8e4de" },
        ].map(s => (
      ...
      {urgent.length > 0 && (
        <div style={{ background: "#ffffff", border: "1px solid #e8e4de", borderRadius: 16, padding: 16, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#a09890", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 12 }}>Upcoming Targets</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {urgent.map(to => {
              ...
              return (
                ...
                >
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: isOd ? "#fef2f2" : "#fff7ed", border: `1px solid ${isOd ? "#fecaca" : "#fed7aa"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: isOd ? "#dc2626" : "#e07d2a" }}>{to.unit_number}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#1a1614", marginBottom: 2 }}>{to.property_name}</div>
                    <div style={{ height: 3, borderRadius: 2, background: "#f0ece6", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: isOd ? "#dc2626" : "#e07d2a", borderRadius: 2 }} />
                    </div>
                  </div>
      ...
      {total === 0 && (
        <div style={{ textAlign: "center", padding: "48px 24px", color: "#a09890" }}>
          <div style={{ fontSize: 40, marginBottom: 12, opacity: 0.4 }}>
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: "0 auto" }}><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
          </div>
